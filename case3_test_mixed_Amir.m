...
for i=1:length(Delta1_vector)
    ...
    ns_out(:,i)=case3_(W,rho_s(:,:,i));
    rho_out_real(:,i)=real(reshape(rho_s(:,:,i),[1,dima1^2*dima1^2]));
    rho_out_imag(:,i)=imag(reshape(rho_s(:,:,i),[1,dima1^2*dima1^2]));
end

csvwrite('rho_out_real.csv', rho_out_real);
csvwrite('rho_out_imag.csv', rho_out_imag);
csvwrite('ns_out.csv', ns_out);
